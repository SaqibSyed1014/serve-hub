...
const { hotelDetails, hotelJobs } = storeToRefs(hotelStore);

const isHotelFetching = ref<boolean>(true);

onMounted(async () => {
  isHotelFetching.value = true;
  await hotelStore.fetchHotelDetails(route.params?.id as string);
  isHotelFetching.value = false;
})

const searchedJob= ref<string>('');

// remove the 'About College tab if no description is found'
watch(() => hotelDetails.value, (val) => {
...
<template>
  <div v-if="isHotelFetching" class="container">
    <div class="flex justify-center items-center h-[calc(100vh-80px)] w-full">
      <BaseSpinner size="lg" :show-loader="isHotelFetching" />
    </div>
  </div>

  <div v-else-if="hotelDetails" class="border-t md:border-b border-gray-200 mb-24">
          ...
          <div class="-mt-5 px-4 lg:px-8">
            ...
            >
              <div class="w-24 h-24 shrink-0 bg-white rounded-[10px] shadow-lg flex justify-center items-center mb-2">
                <template v-if="hotelDetails?.logo_path?.length">
                  <img :src="hotelDetails.logo_path" :alt="hotelDetails.name" class="w-full h-full object-cover">
                </template>

                <SvgoBuilding v-else class="size-14 shrink-0" />
              </div>
              <div class="flex-col justify-start items-start gap-5 inline-flex">
                <BaseTooltip :tooltip-content="hotelDetails.name" id="orgTitle">
              ...
              <div class="flex flex-col gap-2">
                ...
                <div class="inline-flex gap-2">
                  ...
                  <span class="text-slate-600 text-base font-medium leading-normal">
                    {{ `${hotelDetails.city}, ${hotelDetails.state_code}` }} {{  }}
                  </span>
                </div>
              </div>
...
<!--              </div>-->
            ...
            >
              ...
              <div
                  v-if="activeTab === 1 && hotelDetails.job_count"
